...
DeviceFilamentZone::DeviceFilamentZone(wxWindow* parent) : wxPanel(parent, wxID_ANY)
{
    ...
    m_refresh_btn = new ScalableButton(m_panel_title, wxID_ANY, "sync_filament");
    ...
    // --- Content panel (two-column grid, mirrors Filaments double-column layout) ---
    m_panel_content = new wxPanel(this, wxID_ANY);
    m_panel_content->SetBackgroundColour(content_bg);
    // sizer set in refresh(); placeholder vertical sizer for now
