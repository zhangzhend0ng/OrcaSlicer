...
void Preview::update_layers_slider(const std::vector<double>& layers_z, bool keep_z_range)
{
    ...
    m_layers_slider->SetExtruderColors(plater->get_extruder_colors_from_plater_config(wxGetApp().is_editor() ? nullptr : m_gcode_result, /*include_mixed=*/false, /*force_device_palette=*/true));
...
void Preview::load_print_as_fff(bool keep_z_range, bool only_gcode)
{
    ...
    if (gcode_view_type == GCodeViewer::EViewType::ColorPrint) {
        ...
        colors = wxGetApp().plater()->get_extruder_colors_from_plater_config(m_gcode_result, /*include_mixed=*/false, /*force_device_palette=*/true);
