...
void MainFrame::init_menubar_as_editor()
{
    ...
    append_menu_item(editMenu, wxID_ANY, _L("Voxel Editor") + dots, _L("Open AI 3D Voxel Editor for current model"),
        [this](wxCommandEvent&) {
            ...
            dlg->ShowModal();
            if (m_plater) {
                TriangleMesh result = dlg->export_mesh();
                if (!result.empty()) {
                    wxString tmp = wxFileName::CreateTempFileName("voxel_");
                    result.write_binary(tmp.ToStdString().c_str());
                    m_plater->load_files(std::vector<fs::path>{fs::path(tmp.wc_str())});
                }
            }
            dlg->Destroy();
        },
